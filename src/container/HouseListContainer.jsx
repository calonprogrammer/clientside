...
class HouseListContainer extends Component {
	...
	fetchKosts(){
		...
		axios.get(`http://127.0.0.1:8000/owner/getAllHouse`,config).then((response)=>{
			const {data, ...paginate} = response.data.kosts;
			this.setState({kosts: data ,paginate: paginate,isLoading: false});
		}).catch((error)=>{
			console.log(error);
		})
	}
	...
	render() {
		return (
			<div>
				...
				<Container>
					<div id={'title'}>Your Kosts</div>
					{this.state.kosts && this.state.kosts.map((key)=>
						(<KostContainer property={key} delete={(id)=>this.delete(id)} update={(id)=>this.update(id)}/>))}
				</Container>
